...
- New `pending` jobs default to one continuous sealed `source_blueprint` round. Prepare ASR/contact sheet, full and rapid-hook understanding, initial Part storyboards, source-rhythm evidence, and local face evidence concurrently, but treat the preparation report only as an internal checkpoint. In the same round and without user confirmation, author the complete Source Rhythm, run rhythm QC, submit one all-beat visual review, repair and revalidate only failed beats while reusing passed evidence, rebuild rhythm-aware source storyboards, and pass the existing gate. Do not start image or video generation before that gate passes. The full understanding call also emits `people_mode` and `expression_and_gaze`; expression must not trigger another semantic-model call. Cache `expression_prompt_profile.json` by source hash. Single-person prompts use the bounded budget in `rules/EXPRESSION_PROMPT_POLICY.json`; multi-person prompts reuse the existing semantic expression and omit blink instructions. Keep `story_analysis -> storyboard` only for legacy statuses or focused repair.
- Local face evidence must run through `tools/run_face_expression_detector.py`, which owns the project-local `.venv-face-expression` runtime. Never install MediaPipe into the system Python, call `detect_face_expression.py` directly from an arbitrary interpreter, or replace a dependency/detector failure with manually authored frame evidence.
...
- New ASR runs use ElevenLabs Scribe v1 through `rules/ASR_MODEL.json`. Preserve its raw text, word timestamps, speaker turns, sentence segments, and audio events as the evidence layer. Visible subtitles may correct named words. An obvious lexical/context error may use a separate `semantic_review` correction only with a written reason and confidence of at least 0.9; ambiguous wording stays unresolved, and raw ASR evidence is never overwritten.
...
- A source with audible audio always uses the sound-bearing prompt and request branch, including local shot refinement and short experiments. Bind the exact approved-Part audio interval, or the exact source interval only when the experiment has no approved Part, define `@音频1`, write `声音` in every Shot, add concise synchronized `音效` for audible or clearly useful visible actions, and set `generate_audio=true`. The silent format is valid only when current loudness/stream evidence proves the source interval has no audible sound.
...
- Seedance generation model selection is sticky per Job. An explicit Seedance 2.5 request or `seedance25_route_lock.json` delegates the rest of that Job to `.agents/skills/seedance-25-replica/SKILL.md`; ordinary Seedance 2.0 prompts, request builders, EP defaults, submissions, and retries are then out of scope. If the Seedance 2.5 route is unavailable or fails, STOP on that route. Only a later explicit user instruction may replace the lock. For Jobs without a model-family lock, the default route is ordinary `Seedance 2.0` with `model=ep-20260521101914-nwv8j`. Model names are exact: `Seedance 2.0` means ordinary 2.0, `Seedance 2.0 mini` means Mini, and `Seedance 2.0 Fast` means Fast. Request QC must enforce the selected route, and changing the EP must not change the rest of the loop flow.
