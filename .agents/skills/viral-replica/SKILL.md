...
9. New pending jobs use one continuous `source_blueprint` worker/gate round. The round must run the full-video 2fps and opening 0–3s 5fps Wujie Higress analyses concurrently with one ElevenLabs Scribe v1 transcription, then save raw words, speaker turns, word timestamps, sentence segments, audio events, and a measured-cut-aligned rapid-hook timeline. Its preparation report is only an internal checkpoint: without another runner decision or user confirmation, continue to author the complete `剧情分析/source_rhythm.json`, run rhythm QC, submit exactly one all-beat visual review, repair and revalidate only failed beats, rebuild rhythm-aware source storyboards, and run the existing gate. It cannot pass before all of those outputs pass, and no image or video generation may start before the lifecycle advances to `storyboard_passed`. Legacy `story_analysis -> storyboard` statuses remain valid for repair but use the same mandatory video-understanding route.
